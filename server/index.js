...
app.get('/dashboard', auth, async (req, res) => {
  ...
  const [[today]] = await pool.query(`
    SELECT
      ...
      COALESCE(SUM(CASE WHEN payment_type IN ('E-WALLET','GCASH','MAYA') THEN total_amount ELSE 0 END), 0) as ewallet_total,
...
app.get('/reports/daily', auth, async (req, res) => {
  ...
  res.json({
    summary: {
      ...
      ewallet_total:  sum(o => o.payment_type === 'GCASH' || o.payment_type === 'MAYA' || o.payment_type === 'E-WALLET'),
...
// ═══════════════════════════════════════════════════════
// ERROR HANDLER (라우트에서 던진 에러를 JSON으로 응답)
// ═══════════════════════════════════════════════════════
